...
@Component
public class ProductAlarmConfCache {
    @Resource
    ...
    public <T> T getCache(String bizGateId, Class<T> t) {
        Object val = redisUtils.hget(KeyConstance.MESSAGING_PROD_ALARM_CONFIG_CACHE, bizGateId);
        if (null != val) {
    ...
    public void setCache(String bizGateId, Object obj) {
        ...
        redisUtils.hset(KeyConstance.MESSAGING_PROD_ALARM_CONFIG_CACHE, bizGateId, JSON.toJSONString(cache));
    }

    public void clear(String ... biProdIds) {
        redisUtils.hdel(KeyConstance.MESSAGING_PROD_ALARM_CONFIG_CACHE, biProdIds);
    }
}
