package com.landleaf.gateway.security.core.handler;
...
import static com.landleaf.comm.exception.enums.GlobalErrorCodeConstants.FORBIDDEN;
import static com.landleaf.comm.exception.enums.GlobalErrorCodeConstants.SUCCESS;
...
public class AccessDeniedHandlerImpl implements ServerAccessDeniedHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, AccessDeniedException denied) {
        ...
        log.warn("[commence][访问 URL({}) 时，用户({}) 权限不够]", exchange.getRequest().getPath(),
                SecurityFrameworkUtils.getAuthUserId(exchange), denied);
        // 返回 403
        if (exchange.getRequest().getMethod().equals(HttpMethod.OPTIONS)) {
            response.setStatusCode(HttpStatusCode.valueOf(200));
        }
        DataBuffer dataBuffer = response.bufferFactory().wrap(JSON.toJSONBytes(Response.error(FORBIDDEN)));
