...
public interface ProjectStaDeviceWaterMonthMapper extends ExtensionMapper<ProjectStaDeviceWaterMonthEntity> {
    ...
    IPage<WaterMeterPageResponse> selectMeterPage(@Param("page") Page<WaterMeterPageResponse> page, @Param("bizDeviceIds") List<String> bizDeviceIds, @Param("start") Timestamp start, @Param("end") Timestamp end);

    WaterMeterDetailResponse detail(@Param("id") Long id);

    ProjectStaDeviceWaterMonthEntity getManualInsertData(@Param("bizDeviceId") String bizDeviceId);
}
