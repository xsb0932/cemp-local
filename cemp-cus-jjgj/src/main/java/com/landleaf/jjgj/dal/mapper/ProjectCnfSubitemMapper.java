...
import com.landleaf.jjgj.domain.entity.ProjectCnfSubitemEntity;
...
public interface ProjectCnfSubitemMapper extends BaseMapper<ProjectCnfSubitemEntity> {
    ...
    @Select("select t2.* from tb_project_cnf_subitem t1 join tb_project_kpi_config t2 \n" +
            ...
            "where t1.deleted = 0 and t1.parent_id = (select id||'' from tb_project_cnf_subitem where deleted = 0 and parent_id is null limit 1) and t1.project_id = #{bizProjectId} and t2.name not in ('热水单立方用电量','空调单方用电量')")
